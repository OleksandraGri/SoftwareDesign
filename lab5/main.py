...
from task5_composite import Div, P, Span, HTMLIterator, ChangeStyleCommand, VisibleState, HiddenState, StyleVisitor
...
Scene III. A churchyard; in it a Monument belonging to the Capulets.
    ...
    # Використання ітератора для перебору елементів
    iterator = HTMLIterator(div)
    for element in iterator:
        element.render()

    # Команди
    change_style_command = ChangeStyleCommand(span, "color: red")
    change_style_command.execute()

    # Стейт
    visible_state = VisibleState()
    hidden_state = HiddenState()
    visible_state.render(div)
    hidden_state.render(div)

    # Відвідувач
    style_visitor = StyleVisitor()
    style_visitor.visit(span)
